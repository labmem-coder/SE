...
本节按作业要求给出``系统事件 → 责任人''的分工，便于评审追踪。组长部分由唐振桓承担；其余系统事件均匀分给四位组员，统稿、文档美化、交叉检查仍由全组协同。

\subsection{系统事件分工表}

\begin{longtable}{p{2.8cm}p{4.6cm}p{2.4cm}p{2.4cm}}
\toprule
\textbf{类别} & \textbf{系统事件} & \textbf{主要负责人} & \textbf{协作} \\
\midrule
\endhead
充电申请 & \texttt{E\_chargingRequest} & 杨凌俊 & 唐振桓 \\
充电申请 & \texttt{Modify\_Amount} & 杨凌俊 & 袁炜途 \\
充电申请 & \texttt{Modify\_Mode} & 袁炜途 & 杨凌俊 \\
充电申请 & \texttt{Query\_Car\_State} & 刘宇轩 & 杨凌俊 \\
充电申请 & \texttt{Start\_Charging} & 马迪轩 & 唐振桓 \\
充电申请 & \texttt{Query\_Charging\_State} & 马迪轩 & 袁炜途 \\
充电申请 & \texttt{End\_Charging} & 袁炜途 & 唐振桓 \\
查看账单 & \texttt{Request\_Bill} & 刘宇轩 & 袁炜途 \\
查看详单 & \texttt{Request\_DetailedList} & 刘宇轩 & 袁炜途 \\
运行充电桩 & \texttt{powerOn} & 马迪轩 & 杨凌俊 \\
运行充电桩 & \texttt{setParameters} & 马迪轩 & 袁炜途 \\
运行充电桩 & \texttt{Start\_ChargingPile} & 杨凌俊 & 唐振桓 \\
运行充电桩 & \texttt{powerOff} & 杨凌俊 & 马迪轩 \\
查看充电桩状态 & \texttt{Query\_PileState} & 刘宇轩 & 马迪轩 \\
查看队列状态 & \texttt{Query\_QueueState} & 袁炜途 & 刘宇轩 \\
调度交互（组长） & 优先级调度 & \textbf{唐振桓} & 全组 \\
调度交互（组长） & 时间顺序调度 & \textbf{唐振桓} & 全组 \\
调度交互（组长） & 充电中故障恢复 & \textbf{唐振桓} & 全组 \\
